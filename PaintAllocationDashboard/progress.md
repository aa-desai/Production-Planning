...
`Python Script\paint_dashboard_V2_design.md` + the runlist feature). Project overview / IT
handoff: root [README.md](../README.md) (absorbed `graph_allocator_V2_README.md`). UI visual reference:
...
- **V2.1.0 — current (2026-06-10):** `__version__` bumped `2.0.0-dev`→`2.1.0` and exes rebuilt. Adds, over the
  V2.0.x user-testing line: multi-release queue selection + combined ordered push; 5-min reconcile heartbeat +
  last-EC/PC-op removal gate; reworked editor (derived colour→part grouping, FLIP drag, collapse, ack, Reset-to-live);
  toast notifications; loading/click feedback on all buttons; publish-when-empty; EC-eligibility case-sensitivity fix.
  WIP folder promoted to `PaintAllocationDashboard\` (2026-06-10); root README.md is the project/IT-handoff doc.
...
- `paint_dashboard.__version__` → **2.1.0**, dropping `-dev` (promoted to production). MINOR bump
  ...
  gate, editor rework (grouping/collapse/ack/reset), toasts, universal button feedback,
  publish-when-empty; plus the EC-eligibility fix. DESIGN header/§V timeline + status-at-a-glance
  updated. All three exes rebuilt via `build-PaintAllocationDashboard.ps1`; exe smoke shows v2.1.0.

### 2026-06-10 — README: Part Attributes provenance corrected
- README input table + rough-edges updated: `Part Attributes SQL.xlsx` is pulled by a **database
  query** (paint colour as a part attribute); `Part Attributes - Manual.xlsx` **overrides missing
  colours** with manual inputs. Code behaviour documented: folder files are concatenated and per
  part the first **non-null** colour wins (`build_part_paint_flags` sorts `na_position="last"`,
  `drop_duplicates keep="first"`), so the manual file fills SQL gaps; no colour anywhere →
  `Not Found`. (Replaces the old "no Plex report — maintained by hand" claim.)

### 2026-06-10 — Root README.md (project readme + IT handoff)
- New root `README.md` — the GitHub-primary readme and IT handoff: system overview (data flow,
  4 exes), allocation logic, runlist logic (push/eligibility/cascade/EC-deficit/lock/reconcile),
  module-responsibility tables (`paint_dashboard\*` + `Python Script\*`), full UI button
  reference, input-data table (folders/Plex reports/key columns), outputs + runtime files,
  run/build/dev matrix, rough-edges list.
- **Absorbed + removed** `Python Script\graph_allocator_V2_README.md` (full text remains in git
  history); DESIGN.md/PROGRESS.md header references repointed. The two engine `.py` docstrings
  still naming the old file were left untouched (trusted source + byte-identical vendored copy
  must not be edited).

### 2026-06-10 — WIP promoted to production + repo cleanup
- **`PaintAllocationDashboard WIP\` → `PaintAllocationDashboard\`** (the V2 package is now THE
  dashboard). Old v1 folder moved to `Archive\PaintAllocationDashboard (v1, replaced 2026-06-10)\`;
  pre-promotion zip + a stray foreign-machine draft also parked in `Archive\`.
- **Cleanup:** all `*.log`/`*_fault.log` + `__pycache__` purged from the package folder and
  `Python Script\`; root build logs removed.
- **Build/batch:** `build-PaintAllocationDashboard WIP.ps1` renamed to
  `build-PaintAllocationDashboard.ps1` (paths repointed; old script deleted). Legacy
  `build-InventoryAllocator.ps1` + `InventoryAllocator.exe` deleted; `build.ps1` now builds
  GraphAllocator + the dashboard only. `Launch * .bat` (exe launchers) now resolve to the promoted
  folder unchanged; `Launch Paint Allocation Dashboard V2.bat` repurposed as the from-source DEV launcher.
- **`Python Script\` keep-set:** `graph_allocator_V2.py`, `graph_visualize_V2.py`,
  `allocation_common.py` (allocator dep), **`inventory_to_release_allocation.py` KEPT — it is
  imported by `graph_visualize_V2.py`**, README + `allocation_config.ini`.
- **Verified:** exes in the promoted folder are current (built 2026-06-10 08:16 > last source change
  08:11); headless exe smoke — serves, UI carries relselbox/btnRun/toast/ack/reset, draft endpoint OK.
