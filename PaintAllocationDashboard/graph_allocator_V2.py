...
# Only add the on-disk source dir to sys.path when running as a script. A frozen
# .exe carries the shared module bundled inside it; pointing sys.path at the
# OneDrive source would make it import the on-disk copy instead — which on a
# coworker's machine is often a OneDrive "online-only" placeholder that fails to
# read (OSError [Errno 22]).
