...
function Build-Exe([string]$name, [string]$entry, [string[]]$extra) {
  ...
  # Output goes BESIDE the WIP entry scripts (this is the WIP build — it must not overwrite
  # the live PaintAllocationDashboard\ exes). The WIP folder is where these exes are run from.
  $src = Join-Path $dist "$name.exe"
  $dst = Join-Path ".\PaintAllocationDashboard WIP" "$name.exe"
  ...
  Write-Host "Built $name.exe -> PaintAllocationDashboard WIP\ ($((Get-Item $dst).LastWriteTime))"
}
...
Build-Exe "PaintAllocationDashboard" "PaintAllocationDashboard WIP\paint_allocation_dashboard.py" `
...
Build-Exe "PaintRunlistPC" "PaintAllocationDashboard WIP\runlist_pc.py" @()
Build-Exe "PaintRunlistEC" "PaintAllocationDashboard WIP\runlist_ec.py" @()
